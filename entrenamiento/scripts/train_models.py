...
COORD_FILE = (
    PROJECT_ROOT
    / "resultados/analisis_regiones/dataset_entrenamiento_1/analisis/template_analysis_results.json"
)
MODELS_DIR = PROJECT_ROOT / "resultados/entrenamiento/dataset_entrenamiento_1/models"
OUTPUT_DIR = (
    PROJECT_ROOT
    / "resultados/entrenamiento/dataset_entrenamiento_1/visualization_results"
)
LOG_FILE = (
    PROJECT_ROOT / "resultados/entrenamiento/dataset_entrenamiento_1/training.log"
)
...
def train_models(
    ...
    image_processor = ImageProcessor(
        base_path=base_path, template_data_path=coord_file, output_dir=output_dir
    )
    ...
    for coord_name in coord_manager.get_all_coordinate_names():
        ...
        try:
            ...
            if use_kernel:
                pca.train(
                    training_images, kernel=kernel_type, kernel_params=kernel_params
                )
            else:
            ...
            if "explained_variance_ratio" in model_info:
                logging.info(
                    f"Varianza explicada: {model_info['explained_variance_ratio']:.4f}"
                )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Entrenamiento de modelos PCA/Kernel PCA"
    )
    ...
    parser.add_argument(
        "--degree", type=int, default=3, help="Grado del polinomio para kernel Poly"
    )
    ...
    try:
        ...
        for coord_name, info in training_info.items():
            ...
            if "explained_variance_ratio" in info:
                logging.info(
                    f"  Varianza explicada: {info['explained_variance_ratio']:.4f}"
                )

            if args.use_kernel:
                logging.info(
                    f"  Tipo de kernel: {info.get('kernel_type', 'desconocido')}"
                )
