...
output_dir = base_dir / "resultados/prediccion/dataset_1_bresenham_1"
...
def calculate_perpendicular_lines(points, line_slope, length=10):
    """Calcula líneas perpendiculares en cada punto dado."""
    perpendicular_lines = []
    
    for x, y in points:
        if line_slope == 0:  # Línea horizontal
            # Línea perpendicular vertical
            p1 = (x, y - length)
            p2 = (x, y + length)
        elif line_slope == float('inf'):  # Línea vertical
            # Línea perpendicular horizontal
            p1 = (x - length, y)
            p2 = (x + length, y)
        else:
            ...
            dx = length / (1 + m_perp**2)**0.5
            dy = m_perp * dx
            
            p1 = (int(x - dx), int(y - dy))
            p2 = (int(x + dx), int(y + dy))
        
        perpendicular_lines.append((p1, p2))
...
for image_name in os.listdir(images_dir):
    if image_name.lower().endswith(('.png', '.jpg', '.jpeg')):
        ...
        if best_coord1_euclidean and best_coord2_euclidean:
            ...
            perpendicular_lines = calculate_perpendicular_lines(quartile_points, slope, length=10)
            for line in perpendicular_lines:
                draw.line([line[0], line[1]], fill=(0, 255, 255), width=1)
