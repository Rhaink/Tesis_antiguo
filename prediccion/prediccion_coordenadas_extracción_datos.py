...
images_dir = base_dir / "dataset" / "dataset_prueba_1"
json_path = (
    base_dir
    / "resultados/region_busqueda/dataset_entrenamiento_1/json/all_search_coordinates.json"
)
output_dir = base_dir / "resultados/prediccion/dataset_entrenamiento_1"
...
def crop_image(image, top_left, size):
    ...
    return image.crop(
        (top_left[1], top_left[0], top_left[1] + size[1], top_left[0] + size[0])
    )
...
def reconstruct_pca(projected_vector, model_data):
    ...
    reconstructed_centered = model_data["pca"].inverse_transform(
        projected_vector.reshape(1, -1)
    )[0]
...
def process_region(image, coords, centroid_local, crop_size, pca_model, metric="mse"):
    ...
    for y_c, x_c in coords:
        ...
        if (
            0 <= top_left_y
            and top_left_y + crop_size[0] <= resized_image_size[0]
            and 0 <= top_left_x
            and top_left_x + crop_size[1] <= resized_image_size[1]
        ):
            try:
                ...
                if metric == "mse":
                    ...
                    error = calculate_euclidean_distance(
                        cropped_region, reconstructed_image
                    )
                else:
...
for image_name in os.listdir(images_dir):
    if image_name.lower().endswith(
        ...
        results[image_name] = {
            "coord1_mse": {
                "coordinate": best_coord1_mse,
                "error": min_error_coord1_mse,
            },
            "coord1_euclidean": {
                "coordinate": best_coord1_euclidean,
                "error": min_error_coord1_euclidean,
            },
            "coord2_mse": {
                "coordinate": best_coord2_mse,
                "error": min_error_coord2_mse,
            },
            "coord2_euclidean": {
                "coordinate": best_coord2_euclidean,
                "error": min_error_coord2_euclidean,
            },
        }

        # Marcar las mejores coordenadas en la imagen
        if best_coord1_mse:
            output_image = mark_coordinate(
                output_image, best_coord1_mse, color=(0, 255, 0)
            )  # Verde
        if best_coord1_euclidean:
            output_image = mark_coordinate(
                output_image, best_coord1_euclidean, color=(255, 0, 0)
            )  # Rojo
        if best_coord2_mse:
            output_image = mark_coordinate(
                output_image, best_coord2_mse, color=(0, 0, 255)
            )  # Azul
        if best_coord2_euclidean:
            output_image = mark_coordinate(
                output_image, best_coord2_euclidean, color=(255, 255, 0)
            )  # Amarillo
