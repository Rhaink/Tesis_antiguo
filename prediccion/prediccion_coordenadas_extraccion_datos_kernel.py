...
models_dir = base_dir / "resultados/entrenamiento/prueba_kernel_poly_2/models"
...
output_dir = base_dir / "resultados/prediccion/prueba_kernel_poly_2"
...
def apply_pca(image_vector, model_data):
    """Aplica la transformación PCA o Kernel PCA a un vector de imagen."""
    float_vector = image_vector.astype(float)
    if model_data.get('use_kernel', False):
        # Para Kernel PCA, no se centra manualmente el vector
        projected = model_data['pca'].transform(float_vector.reshape(1, -1))[0]
    else:
        # Para PCA lineal, se centra el vector usando la media almacenada
        centered_vector = float_vector - model_data['mean_face'].flatten()
        projected = model_data['pca'].transform(centered_vector.reshape(1, -1))[0]
    return projected

def reconstruct_pca(projected_vector, model_data):
    """Reconstruye la imagen desde su proyección PCA o Kernel PCA."""
    if model_data.get('use_kernel', False):
        # Para Kernel PCA, inverse_transform ya produce la imagen reconstruida
        reconstructed = model_data['pca'].inverse_transform(projected_vector.reshape(1, -1))[0]
    else:
        # Para PCA lineal, se suma la media después de aplicar inverse_transform
    ...
    # Asegurarse de que los valores queden en el rango [0, 255]
