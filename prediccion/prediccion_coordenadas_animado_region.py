...
def visualize_iteration(original_img, current_coord, centroid_local, crop_size, iteration, region_name, crop=None, reconstructed=None, error=None, processed_coords=None):
    """
    Crea una visualización de la iteración actual y la guarda como imagen.
    Además, marca todas las coordenadas ya procesadas de manera sutil.
    """
    ...
    # Marcar las coordenadas ya procesadas con un tono más suave
    if processed_coords:
        for coord in processed_coords:
            # Usar un color gris claro para las coordenadas ya procesadas
            if coord != current_coord:  # No marcar la coordenada actual en gris
                img_copy = mark_coordinate(img_copy, coord, color=(200, 200, 200))
    
    # Marcar el centroide actual con un color más destacado
    img_copy = mark_coordinate(img_copy, (y_c, x_c), color=(0, 0, 255))  # Azul para el centroide actual
...
processed_coords_coord1 = []  # Lista para almacenar las coordenadas procesadas
    ...
    if 0 <= top_left_y and top_left_y + coord1_crop_size[0] <= resized_image_size[0] and \
       0 <= top_left_x and top_left_x + coord1_crop_size[1] <= resized_image_size[1]:
        try:
            ...
            # Registrar esta coordenada como procesada
            processed_coords_coord1.append((y_c, x_c))
            ...
            visualize_iteration(
                ...
                error=mse,
                processed_coords=processed_coords_coord1  # Pasar las coordenadas procesadas
            )
...
processed_coords_coord2 = []  # Lista para almacenar las coordenadas procesadas
    ...
    if 0 <= top_left_y and top_left_y + coord2_crop_size[0] <= resized_image_size[0] and \
       0 <= top_left_x and top_left_x + coord2_crop_size[1] <= resized_image_size[1]:
        try:
            ...
            # Registrar esta coordenada como procesada
            processed_coords_coord2.append((y_c, x_c))
            ...
            visualize_iteration(
                ...
                reconstructed=reconstructed_image,
                error=mse,
                processed_coords=processed_coords_coord2  # Pasar las coordenadas procesadas
            )
