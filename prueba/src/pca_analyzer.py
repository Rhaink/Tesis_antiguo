...
class PCAAnalyzer:
    ...
    def analyze_search_region(self, 
        ...
        El proceso:
        1. Valida dimensiones de imagen y puntos de búsqueda
        2. Para cada punto en la región de búsqueda:
           - Valida que el punto y el template estén dentro de los límites
           - Alinea el template usando el punto de intersección como ancla
           - Extrae la región usando las dimensiones originales del template
           - Calcula y normaliza el error de reconstrucción PCA
        ...
        # Validar coordenadas de búsqueda
        if not search_coordinates:
            raise ValueError("Lista de coordenadas de búsqueda vacía")
        ...
        # Almacenar información de coincidencias
        matches = []
        
        for coord in search_coordinates:
            try:
                # 1. Extraer región usando el template alineado con el punto de búsqueda
                ...
                # 2. Verificar dimensiones del recorte
                ...
                # 3. Normalizar imagen para comparación consistente
                cropped_region = cropped_region.astype(float)
                if cropped_region.max() > 0:
                    cropped_region /= cropped_region.max()

                # 4. Calcular reconstrucción PCA
                omega = self.calculate_omega(cropped_region)
                reconstructed = self.reconstruct_image(omega)
                
                # 5. Calcular error de reconstrucción normalizado
                error = self.calculate_error(cropped_region, reconstructed)
                normalized_error = error / (template_height * template_width)
                
                # 6. Almacenar información de la coincidencia
                matches.append({
                    'coord': coord,
                    'error': normalized_error,
                    'reconstruction_quality': 1.0 - (normalized_error / cropped_region.size)
                })
                
                errors.append(normalized_error)

                # 7. Actualizar mejor coincidencia si corresponde
                ...
                print(f"Error en coordenada {coord}: {str(ve)}")
