...
sys.path.append(str(PROJECT_ROOT / "prueba"))
...
def predict_points(image_path: str,
    ...
    for coord_name, model in models.items():
        try:
            ...
            # Analizar región de búsqueda para encontrar la mejor coincidencia
            min_error, best_search_coord, errors = model.analyze_search_region(
                image=image,
            ...
            # Las coordenadas best_search_coord son donde el template mejor coincide
            # y donde el punto de intersección se alinea correctamente
            logging.info(f"Error mínimo: {min_error:.4f}")
            logging.info(f"Punto de búsqueda óptimo: {best_search_coord}")
            logging.info(f"Paso: {min_error_step} de {len(search_coordinates)}")
            
            # Guardar resultados detallados incluyendo:
            # - Coordenadas del punto de búsqueda óptimo
            # - Datos del template y punto de intersección para visualización
            # - Métricas de error y pasos de búsqueda
            results[coord_name] = {
                'min_error': float(min_error),
                'min_error_coords': best_search_coord,  # mantener nombre del campo por compatibilidad
            ...
            # Visualizar camino de búsqueda y punto final
            visualizer.plot_search_path(
                search_coordinates=search_coordinates,
                best_point=best_search_coord,  # punto donde el template coincide mejor
                coord_name=coord_name,
                save=True
            )
...
def save_results(results: Dict, output_file: str):
    ...
    """
    # Convertir resultados a formato serializable JSON
    serializable_results = {}
    for coord_name, result in results.items():
        serializable_results[coord_name] = {
            'min_error': float(result['min_error']),  # asegurar que sea float
            'min_error_coords': list(result['min_error_coords']),  # convertir tupla a lista
