...
def process_images(coord_manager: CoordinateManager,
    ...
    results = {coord_name: {'processed': 0, 'failed': 0, 'errors': []}
               for coord_name in coord_manager.coord_data.keys()} # Usar las claves de coord_data que tiene la config
    ...
    try:
        ...
        for coord_name, config in coord_manager.coord_data.items():
            ...
            if not all(k in config for k in ["sup", "inf", "left", "right", "width", "height"]):
                 print(f"Advertencia: Configuración incompleta para {coord_name}. Saltando este landmark.")
                 results[coord_name]['failed'] = total_images # Marcar todas como fallidas para esta coord
                 results[coord_name]['errors'].append("Configuración de límites/dimensiones incompleta.")
                 continue
            ...
            try:
                ...
                print(f"Zona de Búsqueda - Intersección (esquina sup-izq): ({intersection_x}, {intersection_y})")
                print(f"Zona de Búsqueda - Dimensiones (bbox): {config['width']}x{config['height']}")
                print(f"Usando datos precalculados del template para {coord_name}...") # Añadido para claridad

            except Exception as e:
                print(f"Error calculando centro/intersección para {coord_name}: {e}. Saltando landmark.")
                results[coord_name]['failed'] = total_images
                results[coord_name]['errors'].append(f"Error en cálculo de centro/intersección: {e}")
                continue
            ...
            for index, coords_dict in coord_manager.coordinates.items():
                try:
                    ...
                    if coord_name not in coords_dict:
                        print(f"Advertencia: No se encontraron coordenadas para {coord_name} en imagen {index}. Saltando.")
                        results[coord_name]['failed'] += 1
                        results[coord_name]['errors'].append(f"Faltan datos de {coord_name} para img {index}")
                        continue
                    ...
                    image = image_processor.load_and_resize_image(image_path, size=(64, 64)) # Asegurar tamaño

                    # Crear la matriz de la región de búsqueda (usando datos del JSON)
                    search_region_matrix = np.zeros((64, 64), dtype=np.uint8) # Usar uint8 es suficiente
                    search_coords = coord_manager.get_search_coordinates(coord_name.lower()) # Busca por 'coordX'
                    if not search_coords:
                         print(f"Advertencia: No se encontraron coordenadas de búsqueda para {coord_name.lower()} en el JSON. Saltando imagen {index}.")
                         results[coord_name]['failed'] += 1
                         results[coord_name]['errors'].append(f"Faltan search_coords JSON para {coord_name} (img {index})")
                         continue
                    ...
                    for y_sr, x_sr in search_coords:
                        ...
                        if 0 <= y_sr <= 63 and 0 <= x_sr <= 63:
                            ...
                            print(f"Advertencia: Coordenada de búsqueda fuera de rango [0, 63] para {coord_name}: ({x_sr}, {y_sr})")


                    # --- Extracción de la Región ---
                    if index % 50 == 0 or processed_count_coord < 5: # Imprimir más detalles al inicio o cada 50
                         print(f"\nProcesando Imagen {index} para {coord_name}:")
                         print(f"  Path: {Path(image_path).name}")
                         print(f"  Punto etiquetado (float original): ({float_x:.2f}, {float_y:.2f})")
                         print(f"  Punto etiquetado (int usado): ({labeled_x_int}, {labeled_y_int})")
                    ...
                    try:
                        ...
                        cropped_image = image_processor.extract_region(
                            ...
                            labeled_point=(labeled_x_int, labeled_y_int), # Pasar tupla de ints
                        ...
                        if cropped_image is None:
                             ...
                             raise ValueError("extract_region devolvió None (error interno en extracción).")
                        # else: # Si no es None, la extracción (aparentemente) funcionó
                        #    if index % 50 == 0 or processed_count_coord < 5: print("  Región extraída OK.")


                    except Exception as e:
                        print(f"Error en extracción para imagen {index}, coord {coord_name}: {str(e)}")
                        results[coord_name]['failed'] += 1
                        results[coord_name]['errors'].append(f"Error en extracción img {index}: {str(e)}")
                    ...
                    if cropped_image is not None:
                        try:
                            ...
                            if success:
                                ...
                                results[coord_name]['failed'] += 1
                                results[coord_name]['errors'].append(f"Error al guardar imagen {index} para {coord_name}")

                        except Exception as e:
                             print(f"Error inesperado al intentar guardar imagen {index} para {coord_name}: {str(e)}")
                             results[coord_name]['failed'] += 1
                             results[coord_name]['errors'].append(f"Error al guardar img {index}: {str(e)}")
                             continue # Saltar al siguiente índice/imagen
                    ...
                    if processed_count_coord > 0 and processed_count_coord % 50 == 0:
                        print(f"  ... {coord_name}: {processed_count_coord} procesadas exitosamente.")

                # --- Fin del try para una imagen individual ---
                except FileNotFoundError as e:
                     print(f"\nError: No se encontró archivo para imagen {index}: {str(e)}")
                     results[coord_name]['failed'] += 1
                     results[coord_name]['errors'].append(f"Archivo no encontrado para img {index}: {str(e)}")
                     ...
                     results[coord_name]['failed'] += 1
                     results[coord_name]['errors'].append(f"Error de valor img {index}: {str(e)}")
                    ...
                    results[coord_name]['failed'] += 1
                    results[coord_name]['errors'].append(f"Error general img {index}: {str(e)}")
                    # Continuar con la siguiente imagen
            ...
            if results[coord_name]['errors']:
                ...
                for i, error in enumerate(results[coord_name]['errors'][:3]): # Mostrar solo los primeros 3
                    print(f"    - {error}")
...
def main():
    ...
    try:
        ...
        COORDINATES_FILE = PROJECT_ROOT.parent / "coordenadas/coordenadas_aligned_entrenamiento_1.csv"
        INDICES_FILE = PROJECT_ROOT.parent / "indices/indices_aligned_entrenamiento_1.csv"
        VISUALIZATION_DIR = PROJECT_ROOT.parent / "resultados/recorte/dataset_aligned_maestro_1"
        ...
        coord_manager.read_search_coordinates(str(PROJECT_ROOT.parent / "resultados/region_busqueda/dataset_aligned_maestro_1/json/all_search_coordinates.json"))
