...
Este módulo implementa el algoritmo SAHS (Statistical Asymmetric Histogram Stretching)
para la normalización del contraste en radiografías de tórax mediante análisis estadístico.
"""
        ...
        try:
            ...
            if max_value != min_value:
                enhanced_image = np.clip(
                    (255 / (max_value - min_value)) * (gray_image - min_value), 0, 255
                ).astype(np.uint8)
            else:
