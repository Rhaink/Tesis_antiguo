...
TESIS_ROOT = Path("/home/donrobot/projects/Tesis")  # Ruta absoluta al directorio Tesis
...
class PulmoAlignViewer:
    def __init__(self, root):
        ...
        # Inicializar directorios y componentes
    ...
    def initialize_directories(self):
        ...
        (TESIS_ROOT / "processed_images").mkdir(parents=True, exist_ok=True)
        (TESIS_ROOT / "visualization_results").mkdir(parents=True, exist_ok=True)
    ...
    def setup_extract_tab(self):
        ...
        self.coord_path = tk.StringVar(value=str(TESIS_ROOT / "coordenadas.csv"))
        ...
        self.indices_path = tk.StringVar(value=str(TESIS_ROOT / "indices.csv"))
    ...
    def collect_extracted_images(self):
        ...
        self.extracted_images = []
        base_dir = TESIS_ROOT / "processed_images"
            ...
            for coord_name, config in self.coord_manager.coord_data.items():
                ...
                for index, coords in self.coord_manager.coordinates.items():
                    try:
                        ...
                        search_coords = self.coord_manager.get_search_coordinates(coord_name.lower())
                        ...
                        self.extract_log_message(f"Punto etiquetado: ({new_x-1}, {new_y-1})")
                        ...
                        try:
                            ...
                            self.extract_log_message("Región extraída exitosamente")
                        except Exception as e:
                            self.extract_log_message(f"Error en extracción: {str(e)}")
                            raise
    ...
    def initialize_components(self):
        try:
            ...
            self.image_processor = ImageProcessor(
                base_path=str(TESIS_ROOT / "COVID-19_Radiography_Dataset"),
                visualization_dir=str(TESIS_ROOT / "visualization_results")
            )
            self.visualizer = Visualizer()
            self.combined_visualizer = CombinedVisualizer()
            
            # Cargar coordenadas de búsqueda
            self.coord_manager.read_search_coordinates(str(TESIS_ROOT / "all_search_coordinates.json"))
    ...
    def select_and_analyze_image(self):
        ...
        initial_dir = str(TESIS_ROOT / "COVID-19_Radiography_Dataset")
    ...
    def analyze_image(self, image_path):
        try:
            ...
            viz_dir = TESIS_ROOT / "visualization_results"
            if viz_dir.exists():
            ...
            for coord_name, config in self.coord_manager.coord_data.items():
                ...
                try:
                    ...
                    search_coordinates = self.coord_manager.get_search_coordinates(coord_name.lower())
