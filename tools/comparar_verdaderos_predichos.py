...
def comparar_coordenadas():
    ...
    df_verdaderos = pd.read_csv("coordenadas/coordenadas_prueba_1.csv", header=None)
    df_verdaderos.columns = [*range(df_verdaderos.shape[1] - 1), "nombre_imagen"]

    # Leer el archivo JSON con las predicciones
    with open(
        "resultados/prediccion/dataset_entrenamiento_1/lote/json/results.json", "r"
    ) as f:
    ...
    for index, row in df_verdaderos.iterrows():
        ...
        coord1_predicho = (
            pred_imagen["coord1_mse"]["coordinate"][1],
            pred_imagen["coord1_mse"]["coordinate"][0],
        )
        coord2_predicho = (
            pred_imagen["coord2_mse"]["coordinate"][1],
            pred_imagen["coord2_mse"]["coordinate"][0],
        )
    ...
    df_resultados.to_csv(
        "resultados/prediccion/dataset_entrenamiento_1/lote/comparacion/resultados_comparacion.csv",
        index=False,
    )
