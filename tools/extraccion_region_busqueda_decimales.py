...
def generate_search_zone(csv_file, coord_num):
    ...
    heatmap = np.zeros((64, 64), dtype=int) # Especificar dtype=int es buena práctica
    print(f"Tipo de coord_x antes del bucle: {coord_x.dtype}") # Diagnóstico opcional
    print(f"Tipo de coord_y antes del bucle: {coord_y.dtype}") # Diagnóstico opcional

    for x_float, y_float in zip(coord_x, coord_y):
        # 1. Redondear al entero más cercano
        x_int = int(round(x_float))
        y_int = int(round(y_float))

        # 2. Asegurar que los índices estén en el rango [0, 63] DESPUÉS de redondear
        #    Esto es importante por si redondear lleva a -1 o 64.
        x_idx = np.clip(x_int, 0, 63)
        y_idx = np.clip(y_int, 0, 63)
        
        # 3. Usar los índices enteros para actualizar el heatmap
