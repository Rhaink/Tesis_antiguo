...
class TemplateAnalyzer:
    def __init__(
        self, search_coordinates_file: str, output_dir: str = "template_analysis"
    ):
        """
    ...
    def calculate_template_distances(
        self, search_region: np.ndarray, template_size: int = 64
        ...
        if a + c >= template_size:
            raise ValueError(
                f"Las distancias verticales a({a})+c({c}) suman más que el tamaño del template"
            )
        if b + d >= template_size:
            raise ValueError(
                f"Las distancias horizontales b({b})+d({d}) suman más que el tamaño del template"
            )
        ...
        for point in coord_points:
            try:
                ...
                if 0 <= x < 64 and 0 <= y < 64:
                    search_region[y, x] = (
                        1  # Mantener orden [y,x] para formato de imagen numpy
                    )
                else:
                    print(
                        f"Advertencia: Ignorando coordenada fuera de rango y={y}, x={x}"
                    )
            except Exception as e:
    ...
    def visualize_template(
        ...
        ax2.plot(0, 0, "r*", markersize=15, label="Origen (0,0)")
        ax2.plot(
            intersection_point[0],
            intersection_point[1],
            "g*",
            markersize=15,
            label=f"Intersección ({intersection_point[0]},{intersection_point[1]})",
        )
        ax2.set_title(f"Template de Recorte - {coord_name}")
        ax2.legend()

        plt.tight_layout()
        plt.savefig(self.output_dir / f"template_analysis_{coord_name}.png", dpi=300)
        plt.close()
        ...
        try:
            ...
            for coord_name, coord_points in self.search_coordinates.items():
                ...
                self.template_data[coord_name] = {
                    ...
                    "distances": {"a": int(a), "b": int(b), "c": int(c), "d": int(d)},
                    "template_bounds": {
                        "min_x": int(d),
                        "max_x": int(d + width),
                        "min_y": int(a),
...
def main():
    ...
    search_coordinates_file = (
        tesis_root
        / "resultados/region_busqueda/dataset_entrenamiento_1/json/all_search_coordinates.json"
    )
    output_dir = (
        tesis_root / "resultados/analisis_regiones/dataset_entrenamiento_1/analisis"
    )

    # Crear y ejecutar analizador
    analyzer = TemplateAnalyzer(
        search_coordinates_file=str(search_coordinates_file), output_dir=str(output_dir)
    )
